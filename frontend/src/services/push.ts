...
export const enableDriverPush = async (driverId: string): Promise<PushSubscription> => {
  ...
  const keyResponse = await cloudFleetApi.get<ApiEnvelope<{ publicKey: string | null }>>('/api/push/public-key');
  if (!keyResponse.data.data.publicKey) throw new Error('Push notifications are not configured on this environment.');
  ...
  await cloudFleetApi.post(`/api/drivers/${encodeURIComponent(driverId)}/push-subscriptions`, subscription.toJSON());
  return subscription;
};
...
export const disableDriverPush = async (driverId: string): Promise<void> => {
  ...
  await cloudFleetApi.delete(`/api/drivers/${encodeURIComponent(driverId)}/push-subscriptions`, { data: { endpoint: subscription.endpoint } });
