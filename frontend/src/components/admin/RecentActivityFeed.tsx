...
export const deriveRecentActivity = (orders: AdminOrder[]): OperationalActivity[] => {
  const activities = orders.flatMap<OperationalActivity>((order) => {
    ...
    if (['IN_PROGRESS', 'ARRIVED', 'DELIVERED', 'DELIVERY_FAILED', 'RETURNING', 'RETURNED'].includes(order.status)) items.push({
    ...
    if (order.status === 'DELIVERED' && order.deliveredAt) {
      ...
      items.push({ id: `${order.orderId}-proof`, kind: 'proof', title: activityLabels.proof, detail: `Proof of Delivery secured for ${shortId}`, occurredAt: timestampOffset(order.deliveredAt, -1_000) });
      items.push({ id: `${order.orderId}-completed`, kind: 'completed', title: activityLabels.completed, detail: `${shortId} delivered successfully`, occurredAt: order.deliveredAt });
    }
    return items;
  });

  return activities.sort((left, right) => right.occurredAt.localeCompare(left.occurredAt)).slice(0, 12);
};
...
export const RecentActivityFeed = ({ activities }: { activities: OperationalActivity[] }) => (
  <section className="overflow-hidden rounded-2xl border border-neutral-200 bg-white">
    <header className="flex items-center justify-between border-b border-neutral-100 px-5 py-4 sm:px-6">
      <div><h2 className="text-sm font-bold">Recent activity</h2><p className="mt-1 text-[10px] text-neutral-400">Latest operational transitions</p></div>
      <span className="text-[9px] font-bold tracking-wider text-neutral-400 uppercase">Derived live feed</span>
    </header>
    <div className="max-h-[28rem] divide-y divide-neutral-100 overflow-y-auto">
      {activities.map((activity) => (
        <article key={activity.id} className="group grid grid-cols-[auto_1fr_auto] items-start gap-3 px-5 py-4 transition hover:bg-neutral-50 sm:px-6">
          <span className={`mt-1.5 size-2 rounded-full ring-4 ring-neutral-100 ${activityDots[activity.kind]}`} />
          <div><p className="text-xs font-bold text-neutral-900">{activity.title}</p><p className="mt-1 text-[11px] leading-4 text-neutral-500">{activity.detail}</p></div>
          <time dateTime={activity.occurredAt} title={new Date(activity.occurredAt).toLocaleString('en-AU')} className="text-[9px] font-medium text-neutral-400">{relativeTime(activity.occurredAt)}</time>
        </article>
      ))}
    </div>
  </section>
