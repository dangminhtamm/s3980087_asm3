  ...
  return (
    <div className="min-h-screen bg-[#f7f7f6] pb-18 text-neutral-950 lg:grid lg:grid-cols-[14.5rem_1fr] lg:pb-0">
          ...
          <div className="flex items-center gap-6">
            <button
              ...
              onClick={() =>
                toast({
                  title: 'No new alerts',
                  description: 'Operational notifications will appear here.',
                  tone: 'info',
                })
              }
              className="relative text-neutral-500 transition hover:text-neutral-950"
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              onClick={() =>
                toast({
                  title: 'No new alerts',
                  description: 'Your operation is running normally.',
                  tone: 'info',
                })
              }
              aria-label="Notifications"
              className="flex size-9 items-center justify-center rounded-full border border-neutral-200 text-neutral-600"
            >
              <AppIcon name="bell" />
            </button>
          </div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-7 border-t border-neutral-200 bg-white/95 px-1 pb-[max(0.45rem,env(safe-area-inset-bottom))] pt-1.5 backdrop-blur lg:hidden">
        {[
          ...navigation,
          { label: 'Settings', to: '/admin/settings', icon: 'settings' as const },
        ].map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
            ...
            className={({ isActive }) =>
              `flex min-w-0 flex-col items-center gap-1 rounded-lg py-1.5 text-[9px] font-bold transition ${isActive ? 'bg-neutral-100 text-neutral-950' : 'text-neutral-400'}`
            }
          >
