...
import { getAnalyticsOverview } from '../../features/analytics/api/analytics.client';
...
type RegionPoint = AnalyticsOverviewData['regions'][number] & { displayRegion: string };
const AnalyticsControls = ({
  ...
  data: AnalyticsOverviewData;
  draft: AnalyticsDateRange;
  setDraft: React.Dispatch<React.SetStateAction<AnalyticsDateRange>>;
  isApplying: boolean;
  isRefreshing: boolean;
  submitRange(event: FormEvent): void;
  selectPreset(days: number | 'all'): void;
}) => (
  <>
...
const VolumeCharts = ({ data }: { data: AnalyticsOverviewData }) => (
...
const RegionCharts = ({
  ...
  regionChartData: RegionPoint[];
  fastest: RegionPoint | null;
  slowest: RegionPoint | null;
  openRegion(region: string): Promise<void>;
}) => (
      ...
      <div className="h-80 px-3 py-6 pr-6">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            >
              {regionChartData.map((region) => (
                <Cell key={region.region} fill={region.successRate < 90 ? '#dc2626' : '#737373'} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
...
const RegionDrawer = ({
  ...
  regionData: AnalyticsOverviewData | null;
  openRegion(region: string): Promise<void>;
}) => (
  <Drawer
