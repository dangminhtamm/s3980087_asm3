...
const driverIcon = (status: DriverStatus): DivIcon => new DivIcon({
...
const AdminOperationsMapComponent = ({ drivers, activeOrders }: AdminOperationsMapProps) => {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-neutral-200 bg-neutral-200">
      <MapContainer center={positions[0] ?? DEFAULT_CENTER} zoom={13} scrollWheelZoom className="cloudfleet-control-map" aria-label="Live fleet operations map">
        ...
        {locatedDrivers.map((driver) => (
          <Marker key={driver.driverId} position={[driver.lat!, driver.lng!]} icon={driverIcons[driver.status]} zIndexOffset={500}>
            <Popup>
              <div className="min-w-44 font-sans">
                <p className="text-sm font-bold">{driver.name}</p>
                <p className="mt-1 text-[11px] text-neutral-500">{driver.driverId} · {driver.vehiclePlate}</p>
                <div className="mt-3"><StatusBadge label={driver.status === 'ON_DELIVERY' ? 'On delivery' : driver.status === 'AVAILABLE' ? 'Available' : 'Offline'} tone={driver.status === 'ON_DELIVERY' ? 'warning' : driver.status === 'AVAILABLE' ? 'success' : 'neutral'} /></div>
              </div>
            </Popup>
          </Marker>
        ))}
          ...
          <Marker key={order.orderId} position={[order.lat, order.lng]} icon={destinationIcon}>
            <Popup>
              <div className="min-w-48 font-sans">
                <p className="text-[9px] font-bold tracking-wider text-neutral-400 uppercase">Active destination</p>
                <p className="mt-1 text-sm font-bold">{order.customerName}</p>
                <p className="mt-1 text-[11px] leading-4 text-neutral-500">{formatLocationInEnglish(order.dropoffAddress)}</p>
                <p className="mt-2 font-mono text-[10px]">CF-{order.orderId.slice(0, 8).toUpperCase()}</p>
              </div>
            </Popup>
          </Marker>
        ))}
      ...
      <div className="pointer-events-none absolute bottom-4 left-4 z-[500] flex flex-wrap gap-2 rounded-xl border border-white/70 bg-white/92 px-3 py-2 shadow-lg backdrop-blur">
        ...
        <span className="flex items-center gap-2 text-[10px] font-semibold text-neutral-600"><span className="size-2 rotate-45 bg-neutral-950" />Destination</span>
      </div>
    </div>
  );
};
