...
const driverIcon = new DivIcon({
  className: 'cloudfleet-map-icon',
  html: '<span class="cloudfleet-map-driver cloudfleet-map-driver--on-delivery"><span></span></span>',
  iconSize: [32, 32],
  iconAnchor: [16, 16],
});
...
const Viewport = ({ positions }: { positions: MapCoordinate[] }) => {
  ...
  useEffect(() => {
    ...
    if (positions.length === 1) map.setView(positions[0]!, 15);
      ...
      map.fitBounds(latLngBounds(positions), { padding: [50, 50], maxZoom: 15 });
  }, [map, positions]);
  return null;
};
...
const DetailLocationMapComponent = ({
  destination = null,
  driver = null,
}: DetailLocationMapProps) => {
  const positions = useMemo(
    () =>
      [destination?.position, driver?.position].filter((position): position is MapCoordinate =>
        Boolean(position),
      ),
    [destination, driver],
  );
  ...
  return (
    <MapContainer
      ...
      className="cloudfleet-detail-map"
    ...
    >
      ...
      {driver && (
        <Marker position={driver.position} icon={driverIcon} zIndexOffset={500}>
          <Popup>{driver.label}</Popup>
        </Marker>
      )}
      {destination && driver && (
        <Polyline
          positions={[driver.position, destination.position]}
          pathOptions={{ color: '#171717', weight: 2, opacity: 0.65, dashArray: '7 8' }}
        />
      )}
