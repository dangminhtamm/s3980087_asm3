...
const driverNavigation: ReadonlyArray<{
  label: string;
  to: string;
  icon: AppIconName;
  end?: boolean;
    ...
    <div className="min-h-screen bg-[#f4f6f8] text-slate-950 lg:grid lg:grid-cols-[15rem_1fr]">
      <aside className="hidden h-screen border-r border-white/10 bg-slate-950 text-white lg:sticky lg:top-0 lg:flex lg:flex-col">
        ...
        <nav className="flex flex-1 flex-col gap-1 p-4">
          <p className="px-3 pb-3 pt-2 text-[9px] font-bold tracking-[0.2em] text-slate-600 uppercase">
            Driver workspace
          </p>
        ...
        <div className="border-t border-white/10 p-4">
          <div className="flex items-center gap-3 rounded-xl bg-white/5 p-3">
            <span className="flex size-9 items-center justify-center rounded-full bg-teal-400 text-xs font-black text-slate-950">
              {initials}
            </span>
        ...
        <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/92 backdrop-blur lg:h-20">
          <div className="mx-auto flex h-17 max-w-7xl items-center justify-between px-4 sm:px-6 lg:h-full lg:px-8">
            ...
            <div className="hidden lg:block">
              <p className="text-[10px] font-bold tracking-[0.17em] text-slate-400 uppercase">
                {today}
              </p>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="relative flex size-9 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-600 transition hover:border-slate-400"
              >
                <AppIcon name="bell" />
              </button>
            </div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-3 border-t border-slate-200 bg-white/95 px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur lg:hidden">
        {driverNavigation.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex flex-col items-center gap-1 rounded-lg py-1.5 text-[10px] font-bold ${isActive ? 'text-slate-950' : 'text-slate-400'}`
            }
          >
