...
const getApiErrorMessage = (error: unknown): string => {
  if (axios.isAxiosError(error)) {
    const providerMessage = (error.response?.data as { error?: { message?: string } })?.error
      ?.message;
...
export const DriverOrderDetails = ({
  ...
  const estimatedMinutes =
    distanceKm === null ? null : Math.max(2, Math.round((distanceKm / 25) * 60));
  const navigationUrl = useMemo(
    () => getNavigationUrl(destination, driverLocation),
    [destination, driverLocation],
  );
  ...
  useEffect(() => {
    let active = true;
    void getPendingProof(order.orderId)
      .then((pending) => {
        ...
        setFeedback({
          type: 'success',
          message: 'Recovered the draft proof saved on this device.',
        });
      })
      .catch(() => undefined);
    return () => {
  ...
  const startRoute = useCallback(async () => {
    ...
    try {
      ...
      if (result.isQueued)
        setFeedback({
          type: 'success',
          message: 'Route start saved offline and will sync automatically.',
        });
    } catch (error: unknown) {
  ...
  const recordArrival = useCallback(async () => {
    ...
    try {
      ...
      if (result.isQueued)
        setFeedback({ type: 'success', message: 'Arrival saved offline and queued for sync.' });
    } catch (error: unknown) {
      setFeedback({ type: 'error', message: getApiErrorMessage(error) });
    }
  ...
  const confirmDelivery = useCallback(async () => {
    ...
    if (!isOnline) {
      ...
      setFeedback({ type: 'error', message: 'Reconnect to confirm this delivery.' });
      return;
    }
    setPhase('confirming');
    setFeedback(null);
  ...
  const reportFailure = useCallback(async () => {
    ...
    try {
      ...
      if (result.isQueued)
        setFeedback({ type: 'success', message: 'Exception saved offline and queued for sync.' });
    } catch (error: unknown) {
      setFeedback({ type: 'error', message: getApiErrorMessage(error) });
    }
    ...
    <main className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 sm:py-10 lg:px-8">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-black tracking-tight sm:text-3xl">CF-{shortOrderId}</h1>
        </div>
        <span
        ...
        >
          ...
          {order.status === 'ASSIGNED'
            ? 'Assigned'
            : order.status === 'ARRIVED' || hasArrived
              ? 'At destination'
              : 'In transit'}
        </span>
      </header>
      ...
      {!isOnline && (
        ...
        >
          <OfflineIcon />
          <p>
            Offline mode. Route events and GPS are queued on this device; proof photos and
            signatures remain here until upload is possible.
          </p>
        </div>
      )}
          ...
          <div className="p-5 sm:p-7 lg:p-9">
            <div className="grid grid-cols-2 gap-3">
              ...
              <Metric
                ...
                value={estimatedMinutes === null ? 'Calculating…' : `${estimatedMinutes} min`}
              />
            </div>
            ...
            {order.status === 'ASSIGNED' ? (
              <section className="mt-6 rounded-2xl bg-slate-950 p-5 text-white">
                ...
                <p className="mt-2 text-sm leading-6 text-slate-300">
                  Starting records the departure and enables live location sharing, even if the
                  event must sync later.
                </p>
                <Button
            ...
            <div className="pointer-events-none absolute right-4 bottom-4 left-4 rounded-2xl border border-white/80 bg-white/92 p-4 shadow-xl backdrop-blur sm:right-auto sm:max-w-sm">
              ...
              <p className="mt-1 text-sm font-bold">
                {distanceKm === null
                  ...
                  : `${formatDistance(distanceKm)} remaining · ETA ${estimatedMinutes} min`}
              </p>
            </div>
          </div>
        </div>
      ...
      {['IN_PROGRESS', 'ARRIVED'].includes(order.status) && (
        <div className="mt-4 flex justify-end">
          <Button variant="ghost" onClick={() => void reportFailure()}>
...
const WorkflowProgress = ({ currentStep }: { currentStep: number }) => (
  <ol
  ...
  return (
    <section className="mt-6">
      ...
      <input
        ...
        capture="environment"
        onChange={onSelect}
        disabled={uploading || confirming || registered}
        className="sr-only"
      />
      {!proofFile && !registered ? (
        <label
        ...
        <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200">
          ...
          <div className="flex items-center justify-between gap-3 p-4">
            <div className="min-w-0">
              <p className="truncate text-xs font-bold">{proofFile.name}</p>
              <p className="mt-1 text-[10px] text-slate-400">
                {(proofFile.size / 1024 / 1024).toFixed(2)} MB · review before upload
              </p>
            </div>
      ...
      {!registered && (
        <div className="mt-4 space-y-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="text-[9px] leading-4 text-slate-400">
            GPS coordinates and accuracy are attached automatically when device location is
            available.
          </p>
        </div>
      )}
      ...
      {!registered && proofFile && !uploading && (
        <Button className="mt-4 w-full" onClick={onUpload} disabled={!isOnline}>
          {phase === 'upload-error' ? 'Retry proof upload' : 'Upload proof securely'}
        </Button>
      )}
      {registered && (
        <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
          ...
          <Button
            ...
            disabled={!isOnline}
          >
            {phase === 'confirm-error' ? 'Retry delivery confirmation' : 'Confirm delivery'}
          </Button>
        </div>
      )}
    </section>
...
const NextDeliveryPreview = ({ order }: { order: DeliveryOrder }) => (
  ...
  return (
    <div className="mt-5 flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 text-[10px] leading-4 text-slate-500">{state.detail}</p>
      </div>
      {active && (tracking.status === 'denied' || tracking.status === 'error') && (
        <button
          type="button"
...
const OrderField = ({ icon, label, value }: { icon: ReactNode; label: string; value: string }) => (
...
const ReceiptField = ({
  ...
  <div className="bg-white p-4">
    <p className="text-[9px] font-bold tracking-wider text-slate-400 uppercase">{label}</p>
    <p className={`mt-2 text-xs font-bold ${mono ? 'font-mono' : ''}`}>{value}</p>
  </div>
);
const FeedbackBanner = ({ feedback }: { feedback: Exclude<Feedback, null> }) => (
  <div
    role={feedback.type === 'error' ? 'alert' : 'status'}
    className={`mt-5 rounded-xl border px-4 py-3 text-xs font-semibold leading-5 ${feedback.type === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-red-200 bg-red-50 text-red-700'}`}
  >
    {feedback.message}
  </div>
);
