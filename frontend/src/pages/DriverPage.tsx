...
import { Button, DataSourceBadge, EmptyState, ErrorState, Skeleton, useToast } from '../components/ui';
...
export const DriverPage = () => {
  ...
  const driverId =
    auth.user?.username || runtimeEnv('VITE_DRIVER_ID') || 'DRV-018';
  ...
  const loadAssignedOrders = useCallback(async (silent = false) => {
  ...
  const pendingOrders = useMemo(
    () => orders
      ...
      .sort((left, right) => {
        if (left.routeId && left.routeId === right.routeId) return (left.stopSequence ?? 999) - (right.stopSequence ?? 999);
        if (left.plannedArrivalAt && right.plannedArrivalAt) return left.plannedArrivalAt.localeCompare(right.plannedArrivalAt);
  ...
  const locationTracking = useDriverLocationTracking(
    driverId,
  ...
  const driverLocation = useMemo<MapCoordinate | null>(() => {
    ...
    return [
      locationTracking.lastLocation.lat,
      locationTracking.lastLocation.lng,
    ];
  ...
  const handleDelivered = useCallback((orderId: string) => {
    ...
    toast({ title: 'Delivery completed', description: 'Proof of Delivery has been secured and the order was updated.', tone: 'success' });
  }, [toast]);

  const handleStarted = useCallback((orderId: string) => {
    setOrders((currentOrders) => currentOrders.map((order) =>
      order.orderId === orderId ? { ...order, status: 'IN_PROGRESS' } : order,
    ));
    toast({ title: 'Route started', description: 'Live location sharing is now active.', tone: 'success' });
  }, [toast]);
  ...
  const handleArrived = useCallback((orderId: string) => {
    ...
    toast({ title: 'Arrival recorded', description: 'You can now capture Proof of Delivery.', tone: 'success' });
  }, [toast]);

  const handleException = useCallback((orderId: string) => {
    setOrders((currentOrders) => currentOrders.map((order) =>
      order.orderId === orderId ? { ...order, status: 'DELIVERY_FAILED' } : order,
    ));
    toast({ title: 'Delivery exception reported', description: 'Dispatch can now reschedule the order or start a return.', tone: 'error' });
  }, [toast]);
  ...
  const trackingLabel = (() => {
    ...
    if (locationTracking.status === 'unsupported') return 'Location is not supported on this device';
  ...
  return (
    <div>
      ...
      {isLoading ? (
        <div className="mx-auto mt-10 max-w-6xl px-4 sm:px-6 lg:px-8"><Skeleton className="h-96 rounded-3xl" /></div>
      ) : displayedOrder ? (
        <DriverOrderDetails
...
const NoAssignedDelivery = ({ driverId }: { driverId: string }) => (
  <main className="mx-auto grid min-h-[32rem] w-full max-w-4xl place-items-center px-4 py-10 sm:px-6 lg:px-8">
    <EmptyState className="w-full rounded-3xl" icon={<span className="text-xl">✓</span>} title="No new deliveries" description={`Driver ${driverId} has no pending or active orders. New deliveries will appear automatically after dispatch assigns them.`} />
  </main>
);
