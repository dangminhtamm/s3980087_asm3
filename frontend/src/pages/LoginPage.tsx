  ...
  return (
    <main className="grid min-h-screen place-items-center bg-neutral-950 px-5 text-white">
      <section className="w-full max-w-md border border-white/15 bg-white/[0.03] p-8 sm:p-10">
        ...
        {error && <p className="mt-5 border border-amber-700/40 bg-amber-900/20 p-3 text-xs text-amber-200">{error}</p>}
        <button
          type="button"
          onClick={() => auth.login(returnTo).catch(() => setError('Unable to open the Cognito sign-in page.'))}
