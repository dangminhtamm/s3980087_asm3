...
test('optimizer prioritizes a time-window breach over a slightly shorter leg', () => {
  ...
  const sequence = optimizeSequence(
    matrix,
    [order('near', 0, 0), order('urgent', 0, 0, '2026-09-10T00:01:00.000Z')],
    departure,
  );
...
test('OSRM matrix and geometry responses retain provider route semantics', async () => {
  const responses = [
    new Response(
      JSON.stringify({
        code: 'Ok',
  ...
  const service = new RoutingService('osrm', 'https://osrm.test', fetcher);
  ...
  assert.equal(plan.plannedDistanceMeters, 1_500);
  assert.deepEqual(plan.geometry[1], [10.78, 106.71]);
});

test('OSRM failure falls back to a usable straight-line plan', async () => {
