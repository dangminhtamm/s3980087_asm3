...
import { customerFeedbackSchema, customerRescheduleSchema } from '../../src/schemas/tracking.schema.js';
...
test('enhanced proof accepts signature, barcode, notes and GPS with strict bounds', () => {
  ...
  assert.equal(registerDeliveryProofSchema.safeParse({ ...parsed, barcode: '<script>' }).success, false);
});

test('customer actions validate rating and a future reschedule window', () => {
  assert.equal(customerFeedbackSchema.safeParse({ rating: 5, comment: 'Great delivery' }).success, true);
  ...
  assert.equal(customerRescheduleSchema.safeParse({ requestedWindowStart: start, requestedWindowEnd: end }).success, true);
  assert.equal(customerRescheduleSchema.safeParse({ requestedWindowStart: end, requestedWindowEnd: start }).success, false);
});
...
test('push service degrades safely when VAPID is not configured', async () => {
  ...
  try {
    ...
    assert.deepEqual(await service.notifyDriver('DRV-001', { title: 'Test', body: 'Test', url: '/driver', tag: 'test' }), { sent: 0, configured: false });
  } finally {
