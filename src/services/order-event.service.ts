...
export class OrderEventService {
  ...
  public async list(order: Order): Promise<OrderEvent[]> {
    ...
    return this.withLegacyEvents(order, recorded).sort((left, right) =>
  ...
  private withLegacyEvents(order: Order, recorded: OrderEvent[]): OrderEvent[] {
    ...
    const add = (
      ...
      events.push({
        eventId: `derived-${type.toLowerCase()}`,
        orderId: order.orderId,
        type,
        occurredAt,
        actorId: 'legacy-snapshot',
        source: 'DERIVED',
        metadata,
      });
    };
    ...
    if (
      ['DELIVERY_FAILED', 'RESCHEDULED', 'RETURNING', 'RETURNED'].includes(order.status) &&
      order.exception
    ) {
      add('DELIVERY_FAILED', order.exception.reportedAt, { reason: order.exception.reason });
    }
    if (order.status === 'RESCHEDULED' && order.exception)
      add('DELIVERY_RESCHEDULED', order.exception.reportedAt);
    if (order.status === 'CANCELLED' && order.exception)
      add('ORDER_CANCELLED', order.exception.reportedAt, { reason: order.exception.reason });
    if (['RETURNING', 'RETURNED'].includes(order.status) && order.exception)
      add('RETURN_STARTED', order.exception.reportedAt);
    if (order.status === 'RETURNED' && order.exception)
      add('ORDER_RETURNED', order.exception.reportedAt);
  ...
  private toEvent(item: Record<string, unknown>): OrderEvent {
    if (
      ...
      throw new Error('Stored order event is invalid');
    const metadata =
    ...
    return {
      ...
      source: item.source === 'DERIVED' ? 'DERIVED' : 'RECORDED',
      metadata,
    };
  }
}
