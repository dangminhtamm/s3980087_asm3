...
import type { CustomerFeedback, CustomerRescheduleRequest, PublicTrackingResponse } from '../domain/entities/tracking.js';
...
export class TrackingService {
  ...
  public async getTracking(token: string): Promise<PublicTrackingResponse> {
    ...
    const feedback = feedbackResult.Item && typeof feedbackResult.Item.rating === 'number' && typeof feedbackResult.Item.submittedAt === 'string'
      ? { rating: feedbackResult.Item.rating, comment: typeof feedbackResult.Item.comment === 'string' ? feedbackResult.Item.comment : null, submittedAt: feedbackResult.Item.submittedAt }
      : null;
    ...
    const distanceRemainingKm = hasFreshDriverLocation && driverResult
      ? haversineKm(
          { lat: driverResult.lat!, lng: driverResult.lng! },
          { lat: order.lat, lng: order.lng },
        )
      : null;
    const roundedDistance = distanceRemainingKm === null
      ? null
      : Math.round(distanceRemainingKm * 100) / 100;
    const estimatedArrivalMinutes = distanceRemainingKm === null || order.status !== 'IN_PROGRESS'
      ? null
    ...
    return {
      ...
      proof: {
        ...
        recipientName: typeof proofResult.Item?.recipientName === 'string' ? proofResult.Item.recipientName : null,
  ...
  public async submitFeedback(token: string, input: { rating: number; comment?: string | undefined }): Promise<CustomerFeedback> {
    const { orderId, tokenHash } = await this.resolveToken(token);
    const order = await this.orders.getOrder(orderId);
    if (order.status !== 'DELIVERED') throw new AppError(409, 'Feedback is available after delivery', 'FEEDBACK_NOT_AVAILABLE');
    const feedback: CustomerFeedback = { rating: input.rating, comment: input.comment ?? null, submittedAt: new Date().toISOString() };
    try {
      await this.database.send(new TransactWriteCommand({ TransactItems: [
        { Put: { TableName: this.tableName, Item: { PK: `ORDER#${orderId}`, SK: 'CUSTOMER#FEEDBACK', ...feedback }, ConditionExpression: 'attribute_not_exists(PK)' } },
        { Put: { TableName: this.tableName, Item: createOrderEventItem({ orderId, type: 'CUSTOMER_FEEDBACK_RECEIVED', occurredAt: feedback.submittedAt, actorId: `customer:${tokenHash.slice(0, 12)}`, metadata: { rating: String(input.rating) } }) } },
      ] }));
    } catch (error: unknown) {
      if (error instanceof TransactionCanceledException) throw new AppError(409, 'Feedback was already submitted', 'FEEDBACK_ALREADY_SUBMITTED');
      throw error;
    }
    return feedback;
  }

  public async requestReschedule(token: string, input: { requestedWindowStart: string; requestedWindowEnd: string; notes?: string | undefined }): Promise<CustomerRescheduleRequest> {
    const { orderId, tokenHash } = await this.resolveToken(token);
    ...
    try {
      await this.database.send(new TransactWriteCommand({ TransactItems: [
        { Update: {
          ...
          ExpressionAttributeValues: { ':request': request, ':pending': 'PENDING', ':assigned': 'ASSIGNED' },
        } },
        { Put: { TableName: this.tableName, Item: createOrderEventItem({ orderId, type: 'CUSTOMER_RESCHEDULE_REQUESTED', occurredAt: request.requestedAt, actorId: `customer:${tokenHash.slice(0, 12)}`, metadata: { requestedWindowStart: request.requestedWindowStart, requestedWindowEnd: request.requestedWindowEnd } }) } },
      ] }));
    } catch (error: unknown) {
      if (error instanceof TransactionCanceledException) throw new AppError(409, 'This delivery can no longer be rescheduled online', 'RESCHEDULE_NOT_AVAILABLE');
      throw error;
    }
    return request;
  }

  private async resolveToken(token: string): Promise<{ orderId: string; expiresAt: number; tokenHash: string }> {
    const tokenHash = hashTrackingToken(token);
    const lookup = await this.database.send(new GetCommand({
      TableName: this.tableName,
