...
export const ORDER_CSV_HEADERS = [
  'orderId', 'customerName', 'customerPhone', 'dropoffAddress', 'region', 'lat', 'lng',
  'status', 'driverId', 'timeWindowStart', 'timeWindowEnd', 'packageWeightKg',
  'packageVolumeM3', 'serviceDurationMinutes', 'routeId', 'stopSequence', 'createdAt', 'deliveredAt',
] as const;
...
export const parseOrderCsv = (text: string): Array<{ row: number; input: Record<string, unknown> }> => {
  ...
  return rows.slice(1).map((values, index) => {
    ...
    const input: CreateOrderInput = {
      ...
      dropoffAddress: record.dropoffAddress!, region: record.region!,
