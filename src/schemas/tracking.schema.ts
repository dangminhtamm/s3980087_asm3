...
export const customerRescheduleSchema = z.object({
  ...
  if (value.requestedWindowStart >= value.requestedWindowEnd) {
    context.addIssue({ code: 'custom', path: ['requestedWindowEnd'], message: 'requestedWindowEnd must be after requestedWindowStart' });
  }
  if (Date.parse(value.requestedWindowEnd) <= Date.now()) {
    context.addIssue({ code: 'custom', path: ['requestedWindowEnd'], message: 'requested delivery window must be in the future' });
  }
});
