...
export class RouteController {
  ...
  public list = async (request: Request, response: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      response
        .status(200)
        .json({ data: await this.routes.listRoutes(input.driverId, input.limit) });
    } catch (error: unknown) {
      next(error);
    }
  };
  ...
  public reorder = async (
    ...
    try {
      ...
      response.status(200).json({
        data: await this.routes.reorder(id, orderIds, request.authenticatedUser!.username),
      });
    } catch (error: unknown) {
      next(error);
    }
  ...
  public reoptimize = async (
    ...
    try {
      ...
      response.status(200).json({
        data: await this.routes.reoptimize(id, departureAt, request.authenticatedUser!.username),
      });
    } catch (error: unknown) {
      next(error);
    }
