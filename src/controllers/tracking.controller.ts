...
import { customerFeedbackSchema, customerRescheduleSchema, trackingTokenParamsSchema } from '../schemas/tracking.schema.js';
...
export class TrackingController {
  ...
  public submitFeedback = async (request: Request, response: Response, next: NextFunction): Promise<void> => {
    try {
      const { trackingToken } = trackingTokenParamsSchema.parse(request.params);
  ...
  public requestReschedule = async (request: Request, response: Response, next: NextFunction): Promise<void> => {
    try {
      const { trackingToken } = trackingTokenParamsSchema.parse(request.params);
      const input = customerRescheduleSchema.parse(request.body);
      response.status(202).json({ data: await this.tracking.requestReschedule(trackingToken, input) });
    } catch (error: unknown) { next(error); }
  };
}
